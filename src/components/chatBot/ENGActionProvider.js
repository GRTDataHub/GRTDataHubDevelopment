...
// Rewards Start 
handleRewards() {
  const message = this.createChatbotMessage(<a>Indexing rewards are collected and distributed whenever Indexers close their allocations. While they are active, allocations keep accruing rewards continuously. Indexers can either force close their allocations manually or let them be closed automatically after the maximum allocation lifetime (every 28 epochs).<br></br><br></br>28 epoch = ~26 days<br></br><br></br>Allocations are continuously accruing rewards while they're active. Rewards are collected by the indexers, and distributed whenever their allocations are closed. That happens either manually, whenever the indexer wants to force close them, or automatically every maximum 28 epochs - max allocation lifetime.<hr></hr>To see your pending rewards, visit <a className="cla" href="https://www.graphtronauts.com/#/profile" target="_blank" rel="noreferrer">Graphtronauts</a> app and add a wallet.</a>
    ,
  );
  this.setState((prev) => ({
    ...prev,
...
// Allocations Start
handleAllocations() {
  const message = this.createChatbotMessage(<a>Allocations are continuously accruing rewards while they're active. Rewards are collected by the indexers, and distributed whenever their allocations are closed. That happens either manually, whenever the indexer wants to force close them, or automatically every maximum 28 epochs - max allocation lifetime.</a>
    ,
  );
  this.setState((prev) => ({
    ...prev,
