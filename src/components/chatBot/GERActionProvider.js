...
// Buy Start
handleBuy() {
  const message = this.createChatbotMessage(<a>Sie können den Preis und alle offenen Märkte auf <a className="cla" href="https://coinmarketcap.com/currencies/the-graph/markets/" target="_blank" rel="noreferrer">CoinMarketCap</a> sehen.</a>
    ,
  );
  this.setState((prev) => ({
    ...prev,
...
handleGRTTokenAddress() {
  const message = this.createChatbotMessage(<a>GRT Token-Adresse: 0xc944e90c64b2c07662a292be6244bdf05cda44a7</a>
    ,
  );
  this.setState((prev) => ({
    ...prev,
...
// Undelegating Start
handleUnDelegating() {
  const message = this.createChatbotMessage(<a>Wann immer ein Delegator die Delegierung aufheben möchte, unterliegen seine Token einer 28-tägigen Entbindungsfrist. Das bedeutet, dass sie 28 Tage lang weder ihre Token übertragen noch Belohnungen verdienen können.<br></br><br></br>Die Undelegierungs-/Auftauperiode dient dazu, bestimmte wirtschaftliche Angriffe zu vermeiden. Sie muss mit der maximalen Zuweisungslebensdauer von 28 Epochen übereinstimmen. In Anbetracht dessen, wie das Belohnungssystem derzeit funktioniert, würde ein kürzerer Zeitraum des Nichtdelegierens möglicherweise dazu führen, dass Delegieren über mehrere gleichzeitige Zuweisungen hinweg „doppelt gezählt“ wird.<hr></hr>Eine Epoche dauert ~24 Stunden <hr></hr>Wann auch immer Indexer möchte sie erzwingen (manuell) oder automatisch alle maximal 28 Epochen - maximale Lebensdauer der Zuweisung. Dies ist, wenn Belohnungen verteilt werden. <br></br><br></br>Eine Sache, die Sie ebenfalls berücksichtigen sollten, ist die kluge Auswahl eines Indexers. Wenn Sie sich für einen Indexer entscheiden, der nicht vertrauenswürdig war oder keine gute Arbeit leistet, werden Sie die Delegation aufheben wollen, was bedeutet, dass Sie viele Gelegenheiten verpassen, Belohnungen zu verdienen, was genauso schlimm sein kann wie das Verbrennen von GRT.<br></br><br></br>Um ein Tutorial zum Undelegating von GRT-Tokens von The Graph anzusehen, klicken Sie <a className="cla" href="https://www.youtube.com/watch?v=3GPY5On7vHc" target="_blank" rel="noreferrer">hier</a>.<br></br><br></br>Ich kann Ihnen mit Dokumentation und Anleitung zur Auswahl eines Indexers helfen.</a>
    ,
  );
  this.setState((prev) => ({
    ...prev,
...
</table><hr></hr><strong>GRT Überblick über die Token-Ökonomie</strong><br></br><br></br><table>
<tr className="indexer-table">
...
handleDelegateWithWallet() {
  const message = this.createChatbotMessage(<a>Wenn Sie GRT über Metamask delegieren möchten. Wenn Sie es noch nicht getan haben, laden Sie <a className="cla" href="https://metamask.io/download/" target="_blank" rel="noreferrer">MetaMask herunter.</a>. Wenn Sie dabei Hilfe benötigen, finden Sie <a className="cla" href="https://blog.wetrust.io/how-to-install-and-use-metamask-7210720ca047" target="_blank" rel="noreferrer">hier</a> eine Anleitung. <br></br><br></br>Sie können sich ein Schritt-für-Schritt-Video-Tutorial zum Delegieren von GRT an einen Indexer ansehen, siehe <a className="cla" href="https://thegraph.academy/delegators/how-to-delegate-grt-tokens/" target="_blank" rel="noreferrer">hier</a>.<br></br>Wenn Sie lieber der Textanleitung folgen möchten, können Sie diese <a className="cla" href="https://thegraph.academy/delegators/how-to-delegate-grt-tokens/" target="_blank" rel="noreferrer">hier</a> nachlesen. Deutscher Reiseführer kommt bald!<hr></hr>Sie können <strong>WalletConnect</strong> verwenden, um auf The Graph Protocol zu delegieren.<br></br>Eine Schritt-für-Schritt-Anleitung dazu finden Sie <a className="cla" href="https://thegraph.academy/delegators/delegating-walletconnect/" target="_blank" rel="noreferrer">hier</a>.<br></br><br></br>Anstelle der Soft Wallet von Metamask können Sie auch eine Hard Wallet wie Ledger verwenden, um Ihre GRT zu delegieren. Hard Wallets können eine weitere Sicherheitsebene hinzufügen, da sie normalerweise eine physische Gerätebestätigung von Transaktionen erfordern. Sie können Metamask über die Metamask Browser Extension mit Ihrer Hardware Wallet verbinden. Stellen Sie sicher, dass die ETH-App auf Ihrem Ledger-Gerät geöffnet ist und Vertragsdaten in der ETH-App auf Ihrem Gerät erlaubt sind.<br></br><br></br>Um eine Hardware-Wallet in Metamask zu verbinden, befolgen Sie diese einfachen Schritte Anweisungen. Wählen Sie die MetaMask-Erweiterung in Ihrem Browser aus, scrollen Sie nach unten und wählen Sie Connect Hardware Wallet.</a>
    ,
  );
  this.setState((prev) => ({
    ...prev,
...
handleCoinbase() {
  const message = this.createChatbotMessage(<a>So delegieren Sie Ihre GRT-Token mit Coinbase Wallet<br></br><br></br>In diesem Artikel erfahren Sie, wie Sie Ihre GRT-Token innerhalb des The Graph-Protokolls mit <a className="cla" href="https://www.coinbase.com/wallet" target="_blank">Coinbase Wallet </a>delegieren.<br></br><br></br>Um die von den Graphtronauten erstellte Anleitung zu lesen, siehe <a className="cla" href="https://medium.com/@OG-Graphtronauts/how-to-delegate-your-grt-tokens-with-coinbase-wallet-85168f84e95e" target="_blank">hier</a>. Deutsche Guides folgen in Kürze! Oder wenn Sie sich lieber ein Video-Tutorial ansehen möchten, finden Sie das unten. Deutsches Video folgt in Kürze, bitte verwenden Sie bei Bedarf Untertitel.<br></br><br></br><Embed className="embed" isDark url='https://www.youtube.com/watch?v=1KU_mVDStIk&t=2s'/></a>
    ,
  );
  this.setState((prev) => ({
    ...prev,
