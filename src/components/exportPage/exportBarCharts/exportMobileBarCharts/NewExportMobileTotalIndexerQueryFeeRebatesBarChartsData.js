...
import { createQueryFactory } from "../../../helpers/graphQLQuery";
import { URLS } from "../../../helpers/config-urls";
import { getDaysInRange } from '../../../helpers/dateParser';

import Highcharts from 'highcharts';
import HighchartsReact from 'highcharts-react-official';

function NewMobileTotalIndexerQueryFeeRebatesBarChartsData() {
...
require('highcharts/modules/export-data')(Highcharts);
    ...
    const fetchDataNow = async () => {
      try {        
        const queryParams = {
          ...
          body : 'totalIndexerQueryFeeRebates'
        };
        ...
        setDataNow(res.data.data.graphNetworkDailyDatas[0].totalIndexerQueryFeeRebates/1000000000000000000);
      } catch (error) {
        console.error(error);
      }
    };
  ...
  useEffect(() => {
    const fetchData7Days = async () => {
      try {        
        ...
        setData7Days(res.data.data.graphNetworkDailyDatas[6].totalIndexerQueryFeeRebates/1000000000000000000);
      } catch (error) {
        console.error(error);
      }
    };
  ...
  useEffect(() => {
    const fetchData28Days = async () => {
      try {        
        ...
        setData28Days(res.data.data.graphNetworkDailyDatas[27].totalIndexerQueryFeeRebates/1000000000000000000);
      } catch (error) {
        console.error(error);
      }
    };
  ...
  const options = {
    ...
    title: {style: {
      ...
      text : 'Total Indexer Query Fee Rebates',
      align: 'center',
      y: 20,
    },
    subtitle: {
    ...
    yAxis: {
      tickInterval: ([dataNow+data7Days+data28Days]/300),
    ...
    [
      {formatter: function() {
        return Highcharts.dateFormat('%d %b %Y',this.value);
      },  name: 'Total Indexer Query Fee Rebates',
      color: {
...
export default NewMobileTotalIndexerQueryFeeRebatesBarChartsData;
