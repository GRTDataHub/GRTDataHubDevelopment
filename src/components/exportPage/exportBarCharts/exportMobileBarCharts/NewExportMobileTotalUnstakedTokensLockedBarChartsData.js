...
function NewMobileTotalUnstakedTokensLockedBarChartsData() {
...
require('highcharts/modules/export-data')(Highcharts);
    ...
    const fetchDataNow = async () => {
      try {        
        const queryParams = {
          ...
          body : 'totalUnstakedTokensLocked'
        ...
        setDataNow(res.data.data.graphNetworkDailyDatas[0].totalUnstakedTokensLocked/1000000000000000000);
      } catch (error) {
        console.error(error);
      }
    };
  ...
  useEffect(() => {
    const fetchData7Days = async () => {
      try {        
        ...
        setData7Days(res.data.data.graphNetworkDailyDatas[6].totalUnstakedTokensLocked/1000000000000000000);
      } catch (error) {
        console.error(error);
      }
    };
  ...
  useEffect(() => {
    const fetchData28Days = async () => {
      try {        
        ...
        setData28Days(res.data.data.graphNetworkDailyDatas[27].totalUnstakedTokensLocked/1000000000000000000);
      } catch (error) {
        console.error(error);
      }
    };
  ...
  const options = {
    ...
    yAxis: {
      tickInterval: ([dataNow+data7Days+data28Days]/10),
      min: [([dataNow+data7Days+data28Days]/3)*0.65],
      title: {
        text: false,
    },
        labels: 
    ...
    [
      {formatter: function() {
        return Highcharts.dateFormat('%d %b %Y',this.value);
      },  name: 'Total Unstaked Tokens Locked',
      color: {
...
export default NewMobileTotalUnstakedTokensLockedBarChartsData;
